...
class matmul_vectorization_asm_test : public ::testing::TestWithParam<matrix_size_t> {};
...
TEST_P(matmul_vectorization_asm_test, equal_matrix) {
    ...
    int *mat_vectorization_asm = (int *)calloc(row_a * col_b, sizeof(int));

    fill_matrix_with_randint(a, row_a, col_a);
    fill_matrix_with_randint(b, col_a, col_b);

    matmul_naive(a, b, mat_naive, row_a, col_a, col_b);
    matmul_vectorization_asm(a, b, mat_vectorization_asm, row_a, col_a, col_b);

    for (size_t i = 0; i < row_a; i++) {
        for (size_t j = 0; j < col_b; j++) {
            EXPECT_EQ(mat_naive[i * col_b + j], mat_vectorization_asm[i * col_b + j])
    ...
    free(b);
    free(mat_naive);
    free(mat_vectorization_asm);
}

INSTANTIATE_TEST_SUITE_P(, matmul_vectorization_asm_test,

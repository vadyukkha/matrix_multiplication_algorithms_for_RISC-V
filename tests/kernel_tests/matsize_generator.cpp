...
std::vector<matrix_size_t> generate_tests_parametrs(size_t count) {
    auto generate_rnd_int = [](int min = 1, int max = std::numeric_limits<int>::max()) {
    ...
    for (size_t i = 0; i < count; i++) {
        int64_t row_a = generate_rnd_int() % 50 + 1;
        int64_t col_a = generate_rnd_int() % 50 + 1;
        int64_t col_b = generate_rnd_int() % 50 + 1;
